...
def test_predict_empty_data():
    with client:
        response = client.get(
            "/predict/", json={"data": [], "feature_names": COL_ORDER}
        )
...
def test_predict_incorrect_amount_of_features():
    with client:
        ...
        response = client.get(
            "/predict/", json={"data": [[0] * 14], "feature_names": req_feats}
        )
        assert response.status_code == 422
        assert (
            response.json()["detail"][0]["msg"]
            == "ensure this value has at most 13 items"
        )
...
def test_predict_incorrect_col_order():
    with client:
        ...
        response = client.get(
            "/predict/", json={"data": [[0] * 13], "feature_names": req_feats}
        )
        assert response.status_code == 400
...
def test_predict_incorrect_feature_format():
    with client:
        ...
        response = client.get(
            "/predict/", json={"data": [req_data], "feature_names": COL_ORDER}
        )
        assert response.status_code == 400
