...
def val(data_dir: str, transformer_dir: str, model_name: str):
    ...
    last_model_data = sorted(client.search_model_versions(f"name='{model_name}'"),
    ...
    model = mlflow.sklearn.load_model(f"models:/{last_model_data.name}/{str(last_model_data.version)}")
    ...
    client.transition_model_version_stage(name=last_model_data.name, # model_name,
                                          version=last_model_data.version, #  version=1,
                                          stage="Production")
